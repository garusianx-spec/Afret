...
### حساب‌های آمادهٔ آزمایش

بدون ثبت‌نام وارد شوید. این حساب‌ها هنگام بالا آمدن سرویس ساخته می‌شوند:

| نقش | شمارهٔ موبایل | رمز عبور | وضعیت |
| --- | --- | --- | --- |
| **مادر باردار** | `09123456789` | `Afrat1404` | هفتهٔ ۱۸، با دادهٔ نمونه در همهٔ تب‌ها |
| پزشک | `09123456780` | `Afrat1404` | برای آزمودن گفتگوی مشاوره از دو طرف |

حساب مادر با ورود اول، دادهٔ نمونه را در مرورگر می‌سازد: سن بارداری هفتهٔ ۱۸
با سونوگرافی تعیین سن، پنج رکورد وزن، شش اندازه‌گیری قند خون (یکی خارج از
محدوده)، سه فشار خون، خلق و تهوع امروز، چک‌لیست نیمه‌کامل، دو خاطره و
چک‌لیست‌های ساک زایمان و سیسمونی.

> **این حساب‌ها در `NODE_ENV=production` ساخته نمی‌شوند.** رمزشان در همین
> فایل منتشر شده است؛ برای فعال کردنشان روی یک محیط staging یک‌بارمصرف،
> `ALLOW_DEMO_ACCOUNTS=true` را ست کنید.
...
| دارایی | کجاست | راهنما |
| --- | --- | --- |
| فونت IranYekan Web (۳ وزن) | `apps/web/public/fonts/` — **در مخزن نیست** | [README](apps/web/public/fonts/README.md) |
...
فونت ایران‌یکان تجاری است و در مخزن نیست. سه فایل زیر را در
`apps/web/public/fonts/` بگذارید:

```
IRANYekanWeb-Regular.woff2    IRANYekanWeb-Medium.woff2    IRANYekanWeb-Bold.woff2
```

بدون آن‌ها هم اپ build و اجرا می‌شود و به Vazirmatn برمی‌گردد.
